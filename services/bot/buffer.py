...
BATCH_SIZE = 2  # Adjust to your preference
...
class BotBuffer:
    ...
    def _get_keys(self, team_id: str):
        ...
        return (
            f"{base}:dirty_list",  # List to preserve thread order
            f"{base}:dirty_set",  # Set for quick lookup
            f"{base}:msg_count",  # Message counter
            f"{base}:thread:",  # Prefix for per-thread message lists
        )
    ...
    def _send_to_updater(self, payload: Dict[str, Any]):
        ...
        print(
            f"✓ Dispatched {len(payload['threads'])} threads for team {payload['team_id']}. Queue length: {queue_length}"
        )
